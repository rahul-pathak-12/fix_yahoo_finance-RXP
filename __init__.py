...
def download(tickers, start=None, end=None, as_panel=False,
    ...
    # format start
    if start is None:
        start = int(_time.mktime(_time.strptime('1950-01-01', '%d/%m/%Y')))
    elif isinstance(start, _datetime.datetime):
        start = int(_time.mktime(start.timetuple()))
    else:
        start = int(_time.mktime(_time.strptime(str(start), '%d/%m/%Y')))
    ...
    if end is None:
        ...
        end = int(_time.mktime(_time.strptime(str(end), '%d/%m/%Y')))
